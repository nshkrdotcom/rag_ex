defmodule Rag.Chunker.Recursive do
  ...
  @doc "Returns default options for the recursive chunker."
  @impl true
  @spec default_opts() :: keyword()
  def default_opts, do: [max_chars: 500, min_chars: nil]

  @doc "Split text using hierarchical recursive chunking."
  @impl true
  ...
  def chunk(%__MODULE__{} = chunker, text, opts) when is_binary(text) do
    ...
    paragraph_spans =
      cond do
        length(spans) > 1 -> spans
        spans == [] -> [{0, byte_size(text)}]
        true -> spans
      end

    paragraph_spans
    |> Enum.flat_map(fn span ->
      chunk_paragraph(span, text, min_chars, max_chars)
    end)
    |> build_chunks(text)
  end

  defp chunk_paragraph({start_byte, end_byte} = span, text, min_chars, max_chars) do
    para_len = span_length(text, span)

    cond do
      para_len <= max_chars ->
        [{start_byte, end_byte, :paragraph}]

      true ->
        segment = binary_part(text, start_byte, end_byte - start_byte)
        sentence_spans = sentence_spans(segment)

        if length(sentence_spans) > 1 do
          sentence_spans
          |> combine_sentences(segment, min_chars, max_chars)
          |> Enum.map(fn {seg_start, seg_end} ->
            {start_byte + seg_start, start_byte + seg_end, :sentence}
          end)
        else
          segment
          |> character_spans(max_chars, 0)
          |> Enum.map(fn {seg_start, seg_end} ->
            {start_byte + seg_start, start_byte + seg_end, :character}
          end)
        end
    end
  end
  ...
  defp build_chunks(spans, text) do
    spans
    |> Enum.with_index()
    |> Enum.map(fn {{start_byte, end_byte, hierarchy}, index} ->
      ...
      Chunk.new(%{
        ...
        end_byte: end_byte,
        index: index,
        metadata: %{chunker: :recursive, hierarchy: hierarchy}
      })
    end)
  end
end
